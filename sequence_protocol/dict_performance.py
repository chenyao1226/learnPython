...
'''为什么dict的性能会这么高，并且查询速度不会因为数据量的增大而变慢
    1. 是因为dict的存储方式是hash表（或者叫散列表）
        1. py会把dict中的key计算出一个hash值，作为在存储中的偏移量（存储位置），所以dict中的key必须都得是可hash的，像list不可hash就不能作为key
        2. 有时候不同的key会计算出相同的hash值，这样就冲突了，py避免这种冲突的方式就是如果有重复的hash值就会多次计算hash，直到没有冲突
        3. dict存储value的时候会预先申请一块连续的内存空间，
'''
